  ...
  return (
    <div className="w-full h-full flex flex-col bg-slide-bg px-12 pt-8 pb-20">
      ...
      <div className="flex-1 flex flex-col justify-center max-w-4xl mx-auto w-full min-h-0">
        ...
        <motion.div
          className="mt-4 bg-amber-500/10 border border-amber-500/30 rounded-2xl p-3"
          initial={{ opacity: 0, scale: 0.95 }}
          animate={{ opacity: 1, scale: 1 }}
          transition={{ delay: 0.9, duration: 0.5 }}
        >
          <h3 className="text-amber-400 font-bold text-sm mb-2 text-center uppercase tracking-wider">Impact</h3>
          <div className="flex gap-3">
            {impacts.map((item, index) => (
              <motion.div
                key={index}
                className="flex-1 flex items-center gap-3 bg-slide-bg rounded-lg px-3 py-2 border border-amber-500/20"
